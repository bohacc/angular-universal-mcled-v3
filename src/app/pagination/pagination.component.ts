...
export class Pagination implements OnInit {
  @Input('itemsOnPage') itemsOnPage: number;
  ...
  ngOnChanges(inputChanges) {
    if (inputChanges.itemsOnPage && isBrowser) {
      this.getPagination();
    }
  }
  ...
  private getPagination() {
    ...
    this.httpTimeout = setTimeout(function () {
      ...
      _this.httpSubscription = _this.http.get(_this.appService.getRootPath() + '/products/list/' + _this.appService.getPath() + '/pagination' + _this.getMeta())
        .subscribe(res => {
          let data = res.json();
